...
func main() {
	...
	err = os.WriteFile("private.key", serverKey, 0600)
	if err != nil {
		log.Fatal(err)
	}

	err = os.WriteFile("cert.pem", serverCert, 0777)
	if err != nil {
		log.Fatal(err)
	}
	...
	err = os.WriteFile("client.key", clientKey, 0600)
	if err != nil {
		log.Fatal(err)
	}

	err = os.WriteFile("client_cert.pem", clientCert, 0777)
	if err != nil {
		log.Fatal(err)
	}
